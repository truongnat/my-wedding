...
export function VenueMap() {
  // Default to ceremony venue for server-side rendering
  const currentVenue = venues[0];

  return (
    <section id="location" className="py-16 bg-gradient-to-br from-green-50 to-blue-50">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-6xl mx-auto">
          {/* Venue Selection - Note: Interactive functionality would need client component */}
          <div className="flex flex-wrap justify-center gap-4 mb-8">
            {venues.map((venue) => (
              <Button
                key={venue.id}
                variant={venue.id === 'ceremony' ? 'default' : 'outline'}
                className={`${
                  venue.id === 'ceremony' ? 'bg-green-500 hover:bg-green-600' : 'hover:bg-green-50'
                }`}
              >
            ...
            {venues.map((venue) => (
              <Card
                ...
                className={`p-4 transition-all duration-300 ${
                  venue.id === 'ceremony' ? 'ring-2 ring-green-500 bg-green-50/50' : 'bg-white/80'
                }`}
              >
                <div className="flex items-center gap-3 mb-2">
                  <Badge
                    variant={venue.id === 'ceremony' ? 'default' : 'secondary'}
                    className={venue.id === 'ceremony' ? 'bg-green-500' : ''}
                  >
                    {venue.type}
                  </Badge>
